(() => {
  ...
  const relatedContainer = document.querySelector('.trending-slider');
  ...
  const getResidence = async (code) => {
    try {
      ...
      console.log(property?.price?.triple);
      console.log(typeof property?.price?.triple);
      price.textContent =
        '₹' + (property?.price?.triple || property?.price?.double || property?.price?.single) + '/mo*';
      area.textContent = `${property?.area?.name || ''}-${property?.location?.name || ''
        }`;
      genderIcon.classList.add(`fa-${property?.gender}`);
      showAmenity(property?.amenity);
      document.querySelector('.price-single').textContent = ` ₹${property?.price?.single || 'Nill'
        }/mo`;
      document.querySelector('.price-double').textContent = ` ₹${property?.price?.double || 'Nill'
        }/mo`;
      document.querySelector('.price-triple').textContent = ` ₹${property?.price?.triple || 'Nill'
        }`;
  ...
  const propNode = (data, idx) => {
    ...
    clone.querySelector('img').src = data?.images[0]
      ? data?.images[0]?.url
      : '';
    clone.querySelector('.prop-slider-card').addEventListener('click', () => {
      window.location.href = `/property-detail/${data?.city?.cityName}/${data?.area?.name || data?.location?.name
        }?code=${data?.code}`;
    });
  ...
  document.querySelector('.explore').addEventListener('click', () => {
    ...
    window.location.href = `/property/${property?.city?.name || 'in'}?city=${property?.city?._id || 'in'
      }`;
  });
  ...
  const share = (whatsapp = true) => {
    const property = GLOBAL_PROPERTY.property;
    const text = `${property?.name || property?.code} in ${property?.city?.cityName || property?.area?.name
      }`;
